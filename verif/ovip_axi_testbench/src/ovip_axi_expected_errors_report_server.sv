`ifndef OVIP_AXI_EXPECTED_ERRORS_REPORT_SERVER__SV
`define OVIP_AXI_EXPECTED_ERRORS_REPORT_SERVER__SV

// Custom UVM report server used by ovip_axi_base_test. Tests register the
// regex of an error they *expect* to fire (via `expected_errors.push_back`);
...
// (each expected pattern is matched against the FIRST live error only, then
// the head is dropped). Tests that don't care about order leave it at 0.

class ovip_axi_expected_errors_report_server extends uvm_default_report_server;
