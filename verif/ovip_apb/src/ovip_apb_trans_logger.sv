`ifndef OVIP_APB_TRANS_LOGGER__SV
`define OVIP_APB_TRANS_LOGGER__SV

// ovip_apb_trans_logger -- writes one text line per completed transfer to a
// per-agent log file (and, optionally, a shared combined file). It subscribes
// to the monitor's analysis_port, which fires once per completed transfer,
// so each line carries the full transfer plus its SETUP timestamp. Works on
// active and passive agents.
//
// The agent instantiates and connects this automatically when
// cfg.enable_trans_log is set; see ovip_apb_agent.
//
...
class ovip_apb_trans_logger extends uvm_subscriber #(ovip_apb_trans);

	`uvm_component_utils(ovip_apb_trans_logger)
	...
	ovip_apb_trans_log_format_e format = OVIP_APB_TRANS_LOG_TABLE;
	...
	function new(string name = "ovip_apb_trans_logger", uvm_component parent = null);
		super.new(name, parent);
	endfunction

	extern virtual function void start_of_simulation_phase(uvm_phase phase);
	extern virtual function void write(ovip_apb_trans t);
	extern virtual function void final_phase(uvm_phase phase);

	// Build the full line: completion time + agent label + the trans's columns.
	protected virtual function string format_line(ovip_apb_trans t);
		string body = (format == OVIP_APB_TRANS_LOG_RAW) ? t.convert2string() : t.log_line();
		return $sformatf("%-11s %-9s %s", $sformatf("%0t", $time), label, body);
	endfunction : format_line

endclass : ovip_apb_trans_logger


function void ovip_apb_trans_logger::start_of_simulation_phase(uvm_phase phase);
	// A factory-created trans so the header matches the actual transaction
	// type via any agent-level type override.
	ovip_apb_trans hdr = ovip_apb_trans::type_id::create("log_hdr");
	...
	begin
		...
		if(fd == 0)
			`uvm_warning("APB_LOG", $sformatf("could not open transaction log '%s'", file_name))
		else if(format == OVIP_APB_TRANS_LOG_TABLE)
	...
	begin
		...
		begin
			...
			if(cfd == 0)
				`uvm_warning("APB_LOG", $sformatf("could not open combined transaction log '%s'", combined_file_name))
			_combined_fd[combined_file_name]   = cfd;
			_combined_refs[combined_file_name] = 0;
			if(cfd != 0 && format == OVIP_APB_TRANS_LOG_TABLE)
...
function void ovip_apb_trans_logger::write(ovip_apb_trans t);
...
function void ovip_apb_trans_logger::final_phase(uvm_phase phase);
